...
<body>
	...
	<div class="x-body">
		...
		<div class="turnPage center fr">
			<span style="margin-right: 20px;">共${page_vo.total}条记录,当前第${page_vo.pageNum }页,共${page_vo.pages}页
			</span> <a href="optioninfo.do?action=search&pageNum=1">首页</a>
			<c:if test="${page_vo.pageNum gt 1 }">
				<a href="optioninfo.do?action=search&pageNum=${page_vo.pageNum-1}">上一页</a>
			</c:if>
			<c:if test="${page_vo.pageNum le 1 }">
				<a href="#">上一页</a>
			</c:if>
			<c:if test="${page_vo.pageNum lt page_vo.pages }">
				<a href="optioninfo.do?action=search&pageNum=${page_vo.pageNum+1}">下一页</a>
			</c:if>
			<c:if test="${page_vo.pageNum ge page_vo.pages }">
				<a href="#">下一页</a>
			</c:if>

			<a href="optioninfo.do?action=search&pageNum=${page_vo.pages}">尾页</a>
